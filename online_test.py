...
# from src.mgm_spfa_zpy import mgm_spfa
    ...
    for i_iter in range(test_iter):
        ...
        for n_graph in range(16, data.num_graphs):
            ...
            if test_mode == "fast-spfa":
                ...
                # X = mgm_spfa(data.K[:n_graph, :n_graph, :, :], X_new, n_graph, data.num_nodes,alt='slf')
                # X = mgm_spfa(data.K[:n_graph, :n_graph, :, :], X_new, n_graph, data.num_nodes,alt='lll')
                # X = mgm_spfa(data.K[:n_graph, :n_graph, :, :], X_new, n_graph, data.num_nodes,alt='dfs')
                X = mgm_spfa(data.K[:n_graph, :n_graph, :, :], X_new, n_graph, data.num_nodes,alt='vec')
            toc = time.time()
