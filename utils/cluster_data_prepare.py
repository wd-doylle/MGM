...
class DataGenerator:
    def __init__(
        ...
        self.num_graphs_cluster = num_graphs_cluster
    ...
    def preprocess(self, coord_data_list=None, shuffle=True):
        if coord_data_list is None:
            coord_data_list = self.coord_data_list
        if shuffle:
            shuffle_list = np.random.permutation(len(coord_data_list))
            coord_data_list = [coord_data_list[shuffle_list[i]] for i in range(len(coord_data_list))]
            self.gt = self.gt[shuffle_list]
    ...
    def add_data(self, data_path, num_inlier=10, num_outlier=2, num_graphs=24):
        data = self.read_mat_and_sample(
            data_path=data_path,
            num_inlier=num_inlier,
            num_outlier=num_outlier,
            num_graphs=num_graphs
        )
        self.coord_data_list.extend(data)
        if self.gt is None:
            self.gt = np.zeros((num_graphs, 1))
        else:
            self.gt = np.concatenate([self.gt, np.ones((num_graphs, 1)) * (self.gt[-1] + 1)], axis=0)

    def read_mat_and_sample(self, data_path=None, num_inlier=10, num_outlier=2, num_graphs=24):
        if data_path is None:
            data_path = self.data_path
        tmp = []
        data_list = [item for item in os.listdir(data_path)]
        data_list.sort()
        for i, item in enumerate(data_list):
            if item == "pairwise_matching.mat":
                mat_path = '{}/{}'.format(data_path, item)
                data = np.array(scio.loadmat(mat_path)['rawMat'], dtype=float)
                m = data.shape[0] // num_inlier
                n = num_inlier
